...
## What's new
* Added the new debugger which is activated with the `-dwin` argument.
...
* Added decorators! Currently only, available decorator is `[local]` which marks a variable as a variable that can be used only within the main thread:
```asm
.data
[local]
num localNumber : 9
...
retn tlr
```
Decorators can be used in both `.data` and `.start` code sections.
